...
$LogDir = Join-Path $Root 'logs\dataset'
$null = New-Item -ItemType Directory -Force -Path $LogDir

$ts = Get-Date -Format 'yyyyMMdd_HHmmss'
$csvPath = Join-Path $LogDir ("{0}_{1}.csv" -f $Action, $ts)

Write-Host "[collect] action=$Action, seconds=$Seconds, port=$Port, baud=$Baud"
Write-Host "[collect] output=$csvPath"
...
$tmp = Join-Path $Root '.codex\collect_action_tmp.py'
$null = New-Item -ItemType Directory -Force -Path (Split-Path -Parent $tmp)
Set-Content -Path $tmp -Value $py -Encoding UTF8

python $tmp --port $Port --baud $Baud --seconds $Seconds --csv $csvPath
if ($LASTEXITCODE -ne 0) {
    throw "collect failed with exit code $LASTEXITCODE"
}

Write-Host "[collect] csv saved: $csvPath"
